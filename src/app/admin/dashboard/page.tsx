  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [agg, tr, top] = await Promise.all([
        fetchAggregate(token, selectedCompany, groupBy),
        fetchTrend(token, selectedCompany),
        fetchTopEmitters(token, selectedCompany),
      ]);
      setAggregate(agg);
      setTrend(tr);
      setTopEmitters(top);
      ...
      <div className="mb-5 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 max-w-[640px] text-[13.5px] text-[var(--text-soft)]">
            Analyse multidimensionnelle des émissions par entreprise.
          </p>
        </div>
        <select
          ...
          <div className="mb-5 grid grid-cols-3 gap-4">
            ...
            <div className="card">
              ...
              <div className="text-[12px] text-[var(--text-faint)]">
                {topEmitters?.items[0]
                  ? `${topEmitters.items[0].percentOfTotal}% du total`
                  : ""}
              </div>
            </div>
          </div>
